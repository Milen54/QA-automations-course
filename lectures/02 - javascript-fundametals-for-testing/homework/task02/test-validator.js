...
function validateResponseTime(actualTime, maxAllowedTime) {
    ...
    console.log(`Response time ${actualTime}ms is within limit ${maxAllowedTime}ms: ${result}`);
    return result;
}
...
function validatePerformanceRange(responseTime, minTime, maxTime) {
    ...
    console.log(`Response ${responseTime}ms is within range ${minTime}-${maxTime}ms: ${result}`);
    return result;
}
...
function compareVersions(currentVersion, requiredVersion) {
    ...
    console.log(`The versions are different: Current version: ${currentVersion} and ${requiredVersion} // ${result}`);
}
...
function validateCompleteAPIResponse(statusCode, responseTime, hasData, errorCount) {
    const check = statusCode === 200 && responseTime < 1000 && hasData === true && errorCount === 0;
...
function complexValidationScenario(statusCode, responseTime, userRole, dataCount, environment) {
