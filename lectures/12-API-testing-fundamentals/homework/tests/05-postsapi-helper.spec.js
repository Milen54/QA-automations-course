...
test("Use API helper for full CRUD workflow", async ({ request }) => {
  ...
  const filteredPosts = await postsAPI.searchPostsByTitle(keyword);

  console.log(`Found ${filteredPosts.length} posts with ${keyword} in title`);
  ...
  filteredPosts.forEach((post) => {
    expect(post.title.toLowerCase()).toContain(keyword);
  });
