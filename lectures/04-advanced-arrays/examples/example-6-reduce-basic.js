...
function countOrderByStatus(statuses) {
  let countOrder = statuses.reduce(
    function (acc, status) {
      if (status === "DELIVERED") {
        acc[0]++;
...
function countVotesByCandidate(candidates) {
  let countVotes = candidates.reduce(
    function (acc, name) {
      if (name === "Alice") {
        acc[0]++;
      } else if (name === "Bob") {
...
function countGamesByOutcome(outcomes) {
  let countOutcomes = outcomes.reduce(
    function (acc, type) {
      if (type === "WIN") {
        acc[0]++;
      } else if (type === "DRAW") {
...
function countShipmentsByStatus(shipmentStatus) {
  let countStatus = shipmentStatus.reduce(
    function (acc, shipStatus) {
...
function averageTimeForPassed(results, times) {
  let [totalTime, passedCound] = results.reduce(
    function ([total, count], result, index) {
      if (result === "PASS") {
        total += times[index];
        count++;
      }
...
let averageAge = people.reduce(
  function ([total, count], [name, age]) {
    if (age >= 18) {
      total += age;
      count++;
    }
...
let [sumEven, countEven] = numbers2.reduce(
  function ([sum1, count1], num) {
    if (num % 2 === 0) {
      sum1 += num;
      count1++;
    }
